...
export default {
  getList: config => {
    const { buildingName, status, department, name, studentid, page = 1, limit = 20 } = param2Obj(config.url)
    const mockList = List.filter(item => {
      if (department !== '全部' && item.department !== department) {
        console.log()
        return false
      }
      ...
      if (buildingName !== '全部' && item.buildingName !== buildingName) {
        return false
      }
